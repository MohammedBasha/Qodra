	...
	class plgSystemWeb357framework extends JPlugin
	{
		...
		public function onAfterInitialise()
		{
			...
			if ($app->isAdmin())
			{
				...
				if ($option == 'com_plugins' || ($option == 'com_modules' || $option == 'com_advancedmodules') || ($option == 'com_config' && $view == 'component' && 
					(
						$component == 'com_vmsales' || 
						$component == 'com_monthlyarchive' || 
						$component == 'com_loginasuser' || 
				...
				))
				{
					...
					// js
					JFactory::getDocument()->addScript(JURI::root(true).'/media/plg_system_web357framework/js/script.min.js?v=1.7.7_20190610183142');

					// css
					JFactory::getDocument()->addStyleSheet(JURI::root(true).'/media/plg_system_web357framework/css/style.min.css?v=1.7.7_20190610183142');
				}
			}
		}
		...
		public function getLangForm($form, $language_name = "English", $lang_code = "en_GB")
		{
			if (isset($form))
			{
				...
				description="PLG_SYSTEM_CPNB_DAYS_DESC" 
				filter="raw" 
				showon="enable_shortcode_functionality:1" 
				/>';
		...
		public function getUsersFormFieldLoginAsUser($form, $usergroup_id, $usergroup_name)
		{
			if (isset($form))
			{
				...
				query="SELECT u.id AS value, CONCAT(u.name, \' (\', GROUP_CONCAT(ug.title), \')\') AS users_'.$usergroup_id.' FROM #__users AS u LEFT JOIN #__user_usergroup_map AS ugm ON u.id = ugm.user_id LEFT JOIN #__usergroups AS ug ON ugm.group_id = ug.id WHERE ug.title != \'Registered\' GROUP BY u.id ORDER BY u.name ASC" 
				multiple="true" 
