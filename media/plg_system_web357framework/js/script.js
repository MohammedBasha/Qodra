...
jQueryWeb357(document).ready(function($) 
{
	 ...
	 */
	var cpnb_path = baseUrl + 'plugins/system/cookiespolicynotificationbar/assets/images/screenshots-for-parameters/';
	screenshots.push({
	 ...
	 */
	var f404_path = baseUrl + 'administrator/components/com_fix404errorlinks/assets/images/screenshots-for-parameters/';
	screenshots.push({
	...
	for (i = 0, len = screenshots.length; i < len; i++)
	{
		...
		if (jversion === 'j4x')
		{
			...
			// styling
			if (sclass === '.cpnb-notification-bar-message' || sclass === '.sh-front_text_available' || sclass === '.sh-front_text_offline') // textarea
			{
				var style = 'margin-left: 20px; cursor: pointer; vertical-align: top;';
			}
			else
			{
		...
		{
			// j3x, j25x
			var screenshot_html = '<div style="display: inline-block; margin-left: 20px; position: relative; top: 2px;"><a href="'+screenshot_src+'" class="hasTooltip modal" data-toggle="modal" data-original-title="Click to see an example." rel="{size: {x: '+screenshots[i].width+', y: '+screenshots[i].height+'}, handler:\'iframe\'}"><i class="icon-eye-open"></i></a></div>';
		}
