...
<div class="defaultLayout" style="width:<?php echo $moduleWidthWrapper;?>; ">
	...
	<div id="btcontentshowcase<?php echo $module->id; ?>"  class="bt-contentshowcase<?php echo $moduleclass_sfx ? ' bt-contentshowcase'.$params->get('moduleclass_sfx'):'';?>">
		...
		<?php if (trim($params->get('moduleclass_sfx'))=="sc_blogpost"){ ?>
			<div class="sc_blogspost_header">
			
				<?php if( trim($params->get('content_title')) ){ ?>
					<div class="sc_blogspost_title"><?php echo $params->get('content_title') ?></div>
				<?php } ?>
				...
				<?php if( trim($params->get('content_desc')) ){ ?>
					<div class="sc_blogspost_desc"><?php echo $params->get('content_desc') ?></div>
				<?php } ?>	
		...
		<?php if (trim($params->get('moduleclass_sfx'))=="sc_right"){ ?>
			<div class="sc_right_header">
			
				<?php if( trim($params->get('content_title')) ){ ?>
					<div class="sc_right_title"><?php echo $params->get('content_title') ?></div>
				<?php } ?>
				...
				<?php if( trim($params->get('content_desc')) ){ ?>
					<div class="sc_right_desc"><?php echo $params->get('content_desc') ?></div>
				<?php } ?>	
					<div style="clear:both;"></div>
				...
				<div class="bt-inner">
					<div class="bt-inner-2">
						<?php if($row->thumbnail && $align_image != 'center'){?>
							<div class="image_float_<?php echo $align_image ;?>">
					...
					<div class="bt-inner-body">					
							...
							<a class="bt-title" target="<?php echo $openTarget; ?>"
								...
								href="<?php echo $row->link;?>"> <?php echo $row->title; ?> </a>
								<?php endif; ?>
								
							<?php if( $showAuthor || $showDate || $show_category_name): ?>
							<div class="bt-extra">
							<?php if( $showAuthor ): ?>
								<span class="bt-author"><?php echo JText::sprintf('BT_INSTRUCTION_SC_CREATEDBY',JHtml::_('link',JRoute::_($row->authorLink),$row->author)); ?>
								</span>
								<?php endif; ?>
								<?php if( $showDate ): ?>
								<span class="bt-date"><?php echo JHTML::_('date', $row->created , 'd M, Y'); ?></span>
								<?php if (trim($params->get('moduleclass_sfx'))=="sc_footer"){ ?>
									<span class="bt-date-recent"><?php echo JHTML::_('date', $row->created , 'M d - Y'); ?></span>
								<?php } ?>
								<?php endif; ?>
								...
								<?php if( $show_category_name ): ?>
									...
									<?php echo JText::_('BT_INSTRUCTION_SC_IN');?>
										<a class="" target="<?php echo $openTarget; ?>"
											title="<?php echo $row->category_title; ?>"
											href="<?php echo $row->categoryLink;?>"><?php echo $row->category_title; ?>
										</a>
									</span>
									<?php else :?>
										<span class="bt-category"> <?php echo JText::_('BT_INSTRUCTION_SC_IN');?><?php echo $row->category_title; ?> </span>
									<?php endif; ?>
								<?php endif; ?>
								<div style="clear:both;"></div>
							</div>
					...
					<?php if (trim($params->get('moduleclass_sfx'))=="sc_blogpost"){ ?>
						<div class="sc_blogpost_extra">
							<a class="sc_blogpost_readmore" target="<?php echo $openTarget; ?>"
							title="<?php echo $row->title;?>"
							href="<?php echo $row->link;?>"> <?php echo JText::_('BT_INSTRUCTION_SC_MORE');?>
						</a>
