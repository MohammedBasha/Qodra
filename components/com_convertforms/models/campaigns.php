...
class ConvertFormsModelCampaigns extends JModelList
{
    ...
    protected function getListQuery()
    {
        ...
        $query->select('
            (select count(c.id) from #__convertforms_conversions as c where c.campaign_id = a.id AND c.state = 1) as leads'
        );

        // Leads Issues
        $query->select('
            (select count(c.id) from #__convertforms_conversions as c where c.campaign_id = a.id AND c.state = 0) as issues'
        );
